...
function processFiles() {
  ...
  while ((fileName = inbox.nextFile())) {
    if (fileName === 'settings.cbor') {
    ...
    } else if (fileName === 'factToSend.cbor') {
      const data = filesystem.readFileSync(fileName, 'cbor');
      ...
      state.factoftheday = data.factToSend;
      state.factoftheday = JSON.stringify(state.factoftheday);
              console.log(JSON.stringify(state));
      updateState();
      callback();


    }else if (fileName === 'saveFacts.cbor') {
      const data = filesystem.readFileSync(fileName, 'cbor');
      ...
      Object.keys(state).forEach((key) => {
        if (typeof data[key] !== 'undefined') state[key] = data[key];
          ...
          if (typeof data.saveFacts !== 'undefined') {
            state.likedFacts.push(data.saveFacts);
          }

        state.likedFacts.forEach(element => {
          console.log(element);
        });
